...
public class MultiThreadFractalRenderer implements Renderer{
        ...
        for (int j = -COUNT_OF_SKIP_RESULTS; j < iterPerSample; j++) {
            ...
            if (symmetry) {
                ...
                for (int k = 0; k < numberOfSymmetry; k++) {
                    ...
                    double rotationPointX = point.x() * Math.cos(angleOfRotation) - point.y() * Math.sin(angleOfRotation);
                    double rotationPointY = point.x() * Math.sin(angleOfRotation) + point.y() * Math.cos(angleOfRotation);
                    if (j < 0 || rotationPointX > 1 || rotationPointY > 1 || rotationPointX < -1 || rotationPointY < -1) {
                        continue;
                    }
                    ...
                    if ((int) rotationPoint.x() < canvas.getWidth() && (int) rotationPoint.y() < (int) canvas.getHeight()) {
    ...
    public FractalImage render(
        ...
        try {
            executorService.awaitTermination(100000000000L, TimeUnit.MICROSECONDS);
